...
void validate_qz(Eigen::Ref<const MatrixXd> qZ) {
  if (!qZ.allFinite())
    throw RuntimeError(
        ErrorCode::InvalidProbabilities,
        ErrorContext{"internal", "qz_validation", -1, static_cast<int>(qZ.cols()),
                     0.0, "qZ contains NaN or Inf values"});
  if ((qZ.array() < -1.0e-12).any())
    throw RuntimeError(
        ErrorCode::InvalidProbabilities,
        ErrorContext{"internal", "qz_validation", -1, static_cast<int>(qZ.cols()),
                     0.0, "qZ contains negative probabilities"});
}
...
[[noreturn]] void throw_runtime_with_context(const VbemOptions &opts,
  ...
  ErrorContext c = ctx;
  c.message = "[" + c.algorithm + "][" + c.phase + "][outer_iter=" +
              std::to_string(c.outer_iteration) + "][clusters=" +
              std::to_string(c.clusters) + "] " + c.message;
...
template <class W, class C>
double vbem(Eigen::Ref<const MatrixXd> X, MatrixXd &qZ, W &weights,
            std::vector<C> &clusters, const double clusterprior,
            const VbemOptions &opts, const int max_iters,
            const int omp_threads) {
  const Index K = qZ.cols();
    ...
    if (std::isfinite(f_prev) && f_prev != 0.0) {
      ...
      if (rel_increase > opts.fenergy_delta)
        throw RuntimeError(
            ...
            ErrorContext{"internal", "vbem", iter, static_cast<int>(clusters.size()),
                         f, "Free energy increase"});
...
bool split_greedy(Eigen::Ref<const MatrixXd> X, const W &weights,
  ...
  for (Index k = 0; k < K; ++k) {
    ...
    ord[static_cast<std::size_t>(k)].tally = tally[static_cast<std::size_t>(k)];
    ord[static_cast<std::size_t>(k)].Fk = clusters[static_cast<std::size_t>(k)].fenergy();
  }
...
#pragma omp parallel for schedule(static)
  ...
  for (const auto &candidate : ord) {
    ...
    const ArrayXb splitk = clusters[static_cast<std::size_t>(k)].splitobs(Xk);
    ...
    W wsplit;
    std::vector<C> csplit;
    (void)vbem<W, C>(Xk, qZref, wsplit, csplit, clusterprior, opts,
                     opts.split_refine_iters, omp_threads);

    if (comutils::anyempty(csplit))
      continue;

    MatrixXd qZaug = comutils::auglabels(k, map_idx, (qZref.col(1).array() > 0.5), qZ);

    const double fsplit = vbem<W, C>(X, qZaug, wsplit, csplit, clusterprior,
                                     opts, /*max_iters=*/1, omp_threads);
...
template <class W, class C>
bool prune_clusters(MatrixXd &qZ, W &weights, std::vector<C> &clusters,
                    const VbemOptions &opts) {
  const Index K = static_cast<Index>(clusters.size());
  ArrayXd Nk(K);
  for (Index k = 0; k < K; ++k)
...
double learn_single(Eigen::Ref<const MatrixXd> X, MatrixXd &qZ, W &weights,
  ...
  while (split) {
    bool pruned = false;
    try {
      f = vbem<W, C>(X, qZ, weights, clusters, clusterprior, opts,
                     opts.max_vbem_iters, omp_threads);
    } catch (const std::runtime_error &e) {
      throw_runtime_with_context(
          opts, ErrorCode::NumericalFailure,
    ...
    try {
      pruned = prune_clusters<W, C>(qZ, weights, clusters, opts);
      split = split_greedy<W, C>(X, weights, clusters, qZ, tally, f, clusterprior,
                                 opts, omp_threads);
    } catch (const std::runtime_error &e) {
      throw_runtime_with_context(
          opts, ErrorCode::SplitFailure,
